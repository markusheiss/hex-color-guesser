...
const outerContainer = document.querySelector('.container-color--outer');
const colorContainer = document.querySelector('.container-color--inner');
...
const handleHealth = function () {
    ...
    if (health <= 0) {
        gameOver = true;
        healthEl.innerHTML = `Game Over! You guessed ${guessedCorrect} ${guessedCorrect > 1 ? 'colors' : 'color'}.`;
...
const resetButtons = function () {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (i === correctButtonIndex)
            btn.innerHTML = guessColor;
        else
            btn.innerHTML = generateHexColor();
